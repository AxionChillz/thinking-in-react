...
- 要素を生成する仕組み に 「純粋関数性」 を強く推奨する 関数型の思想
  ...
  - 宣言的 UI に渡す 要素 を 生成する関数 を コンポーネント と呼ぶ
    ...
    - 関数型言語の 概念 や パターン を 多く取り入れている
  - 具体的には、関数コンポーネントに対して、以下の実装パターンを強く推奨している
    - A. 同じ入力に対して同じ出力を返す関数 (決定的な関数)
      - 同じ state, props, context の組み合わせに対しては、常に同じ React 要素 を返すような関数
      - 擬似的な式で表現すると、
        - f(state, props, context) = React 要素
        - props, state もまとめて状態として考えることもできるため、
        - 簡単に f(state) = UI と表現されることもある
      - 同じ引数が渡されたときに同じ結果を返す関数 = 決定的な関数という (関数型言語の用語)
      - また、 React 公式はこの性質を「idempotent (冪等)」と表現している
        - 厳密な数学的定義とは異なるが、同じ入力に対して同じ出力を返す性質を指す
        - そのため、以後は 冪等な関数 という表現も用いる
      - アンチパターン:
        - 関数内で ランダム値を生成し、その値に依存する
        - 関数内で 現在時刻 を取得し、その値に依存する
        - 関数内で `document.title` の値を取得し、その値に依存する
    - B. 関数の実行で外部の値を変更しない関数 (副作用を持たない関数)
      - React では、以下の処理を 「副作用」 と呼ぶ
        - 外部システムからの値の読み取り
          - `document.title`の値の読み取り
          - fetch による外部サーバからのデータ取得
          - localStorage からのデータ取得 など
        - 外部システムへの値の書き込み
        ...
        - これらの処理は、すべて React 管轄外の外部システム に影響を与える処理 である
        - 副作用とはこのように、関数の実行によって 外部システムの値に影響を与える処理 のことである
      - また、関数型言語の用語では、
        - 関数の実行によって 外部の値に影響を与えない関数 = 副作用を持たない関数 という (関数型言語の用語)
      - React は、関数がこの副作用を持たないことを強く推奨している
      - なお、「要素を生成する関数」が副作用を持たないことを推奨しているだけであり、
        - React で記述するアプリケーション全体に対して 副作用を禁止しているわけではない
        - 副作用を実行するための手段 は 別途 提供されている (後述)
    - A/B の両方を満たす関数 = 純粋関数
      - 関数型コンポーネント とは、UI を形作る関数である
      - この関数が、f(...) = React 要素 という純粋関数性 を保つことが
      - React の設計思想の中心にある
  - なお、これ以降
    - state, props, context を まとめて 状態群 と呼ぶことにする

- 状態群 の値が変化した時に React が React 要素を生成しなおす仕組み

  - 関数コンポーネントは
    - state, props, context の組み合わせ (状態群) から
  ...
  - 状態群 = 関数が依存する入力値 と考えられる
    - state, props, context は 関数コンポーネントが依存する入力値である
    - 入力 が 変化した際に 出力 (React 要素) を 再生成 するという設計
...
- 関数型の考えに基づく 入力値は スナップショット (イミュータブル) なデータとして扱う という設計思想
  ...
  - 余談: JS の仕組みから イミュータブルになっている訳ではない
    - state, props, context 自体は JS のオブジェクトであり、
      ...
      - そのため、技術的には 状態群 を ミュータブルに変更することは可能である
    - しかし、React の設計思想として
      - 関数コンポーネント実行時における 状態群 は
      - スナップショット として扱われ、イミュータブル なデータとして扱われると想定されている
    - そのため、これらの値を 関数コンポーネント実行中に ミュータブルに変更することは React の設計思想に反する
      - 開発者は、これらの状態群 を ミュータブルに変更しないように注意する必要がある

- f(...) = React 要素 という純粋関数性 を保ち、副作用を外部に逃がすための 手段

  - React は f(...) = React 要素 という純粋関数性 を保つことを前提に設計されている
  - React 要素 を生成する関数コンポーネントには、
    - React 管轄外の 外部システム とのやり取り(副作用) の記述 を含めるべきではない
    - React 管轄外 の 外部システム とのやり取りの例
      - `setTimeout` や `setInterval` のタイマー登録
      - 外部サーバへの 接続確立・接続解除
      - 実 DOM ノード の直接操作
        - DOM ノードの直接操作とは、React の管轄外における命令的な操作といえるため、
        - React 管轄外の外部システムといえる
      - ブラウザの API の呼び出し
      - 非 React の 命令的ライブラリ の初期化・破棄 など
  ...
  - 前提知識: useEffect
    ...
    - React 要素から生成された 変更が 実 DOM に反映された後 に
      ...
      - また、依存配列というものを指定することもできる
        ...
        - React が値の変化の検知を行う必要があるため
        - 状態群 (state, props, context) や 関数コンポーネント内で作成した値など
  ...
  - 副作用記述の手段
    ...
    - イベントハンドラ内への副作用記述
      ...
      - このイベントハンドラを登録・実行する手段として、以下の 2 つの手段が提供されている
        ...
        - useEffect による手動での副作用登録
          ...
          - 先程解説したイベントハンドラ属性は
            - 対応する React 要素 が存在しない場合には利用できない
          - 例えば
    ...
    - useEffect 内 への 副作用記述
      ...
      - 実行したい副作用が 特定のイベントに紐づかない場合、β か γ の使い方を用いる
      ...
      - β. React 外部のシステムに対して 初期化・破棄 を行う
        ...
        - 亜種: 外部サーバへのログ送信
          ...
          - この場合は 破棄が必要ない
          - ログ送信の初期化: `fetch` のような関数を呼び出し、ログを送信
          - ログ送信の破棄: 不要
      - γ. React 外部のシステムに対して、 React 内部の state の値を同期させる
        - DOM ノード の直接操作
          - 具体例: `<video>` の再生制御
      ...
      - 余談: 今回解説しなかった useEffect の使い方
        ...
        - 非同期なデータフェッチ (非推奨)
          ...
          - これらのライブラリは
            - データフェッチのロジックを抽象化し
            - キャッシュ管理、再試行、ポーリング、データの同期などの機能を提供している
