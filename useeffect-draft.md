...
- 1b. useEffect を用いて副作用を登録 → DOM ノードがマウント・アンマウントされたという事象に加え、state が変更されたという事象に基づいて副作用を実行
- 2. イベントハンドラ属性を用いてイベントハンドラという副作用を登録 → 任意のイベントに基づいて副作用を実行 (onClick など)

また、この α と β は接続ができ、以下のように合わせて使う場合も存在する

- 3. useEffect を用いて副作用を登録 → DOM ノードがマウント・アンマウントされるイベントに基づいて イベントハンドラという副作用を登録・削除 → 任意のイベントに基づいて副作用を実行
